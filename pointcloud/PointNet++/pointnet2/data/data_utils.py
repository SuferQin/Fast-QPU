...
class PointcloudArbRotate_save(object):
    def __init__(self,gt,loss):
        # self.axis = axis
        self.gt = gt
        self.loss=loss
    ...
    def __call__(self, points):
        ...
        if self.loss=='ce':
            self.gt.append((int(theta/np.pi*36),int(alpha/np.pi*36),int(beta/np.pi*36)))
        else:
            self.gt.append((float(theta/np.pi/2),float(alpha/np.pi),float(beta/np.pi)))
        axis=np.array([
            np.sin(alpha)*np.cos(beta),
            np.sin(alpha)*np.sin(beta),
            np.cos(alpha),
        ])
        rotation_matrix = angle_axis(theta, axis)
